...
service_fields = {
    ...
    "employee": fields.Nested({"id": fields.Integer, "name": fields.String}),
}
    ...
    def get(self):
        if current_user.has_role("admin"):
            customers = Customer.query.filter(
                Customer.roles.any(Role.name != "admin")
            ).all()
            return customers
        else:
...
class ratings_summary_api(Resource):
    @auth_required("token")
    def get(self):
        if not current_user.has_role("emp"):
            return {"message": "Not authorized to view ratings summary"}, 403

        bookings = Booking.query.filter_by(emp_id=current_user.id).all()
        total_ratings = sum(
            booking.rating for booking in bookings if booking.rating is not None
        )
